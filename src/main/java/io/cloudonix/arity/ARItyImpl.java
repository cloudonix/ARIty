 ...
 * @author odeda
 */
public class ARItyImpl implements ARIty, AriCallback<Message> {
	...
	ARItyImpl(Vertx vertx, ARItyOptions options) throws ConnectionFailedException, URISyntaxException {
		this.appName = Objects.requireNonNull(options.appName(), "Application name must be specified");
		this.ce = options.errorHandler();
		var uri = options.uri();
		if (uri == null)
		...
		try {
			ArityARIFactory.setupHttpClient(uri, options.login(), options.password());
			ari = ArityARIFactory.nettyHttp(this.url = uri, options.login(), options.password(), options.ariVersion(), appName, true);
			ari.setHttpClient(options.createHttpClient());
			ari.setWsClient(options.createWsClient());
			logger.info("Ari created {}", url);
			logger.info("Ari version: " + ari.getVersion());
			if (options.openWebSocket()) {
